...
export class Request {
  ...
  static async getWordsList (options:{group?: number, page?: number}) {
    const rawResponse = await fetch(`${url}/words?group=${options.group||0}&page=${options.page||0}`, {
      method: 'GET',
      headers: {
  ...
  static async editUserInfo (id: string, token: string, email: string, password: string) {
    const rawResponse = await fetch(`${url}/users/${id}`, {
      method: 'PUT',
      headers: {
        'Accept': 'application/json',
  ...
  static async getWordFromUserWordsList (id: string, token: string, wordid: string) {
    const rawResponse = await fetch(`${url}/users/${id}/words/${wordid}`, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
  ...
  static async deleteWordFromUsersList (id: string, token: string, wordid: string) {
    const rawResponse = await fetch(`${url}/users/${id}/words/${wordid}`, {
      headers: {
        'Authorization': `Bearer ${token}`,
      },
  ...
  //13. Получить слова определенной группы, определенной страницы c возможностью фильтрации
    static async getAggregatedWordsList (options:{id: string, token: string, group?: number, page?: number, wordsPerPage?: number, filter?: string}) {
      const filterGroup = options.group ? `&group=${options.group}` : "";
      const filterPage = options.page ? `&page=${options.page}` : "";
      const filterWordsPerPage = options.wordsPerPage ? `&wordsPerPage=${options.wordsPerPage}` : "";
      ...
      const rawResponse = await fetch(`${url}/users/${options.id}/aggregatedWords${finalFilter}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json',
  ...
  static async getAggregatedWord (id: string, token: string, wordid: string) {
    const rawResponse = await fetch(`${url}/users/${id}/aggregatedWords/${wordid}`, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
        'Authorization': `Bearer ${token}`,
  ...
  static async editStatistics (id: string, token: string, learnedWords: number, playedGames: number, successRate: string) {
